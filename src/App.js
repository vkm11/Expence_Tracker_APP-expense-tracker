...
function App() {
  ...
  const saveData = async (newExpData) => {
    const resp = await fetch(`http://localhost:3003/saveExpense`, {
      headers: {
  ...
  const deleteRow = async (id) => {
    const resp = await fetch(`http://localhost:3003/deleteExpense/${id}`, {
      method: "DELETE",
    });
  ...
  const updateRow = async (rowData) => {
    ...
    const resp = await fetch(
      `http://localhost:3003/editExpense/${rowData._id}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
